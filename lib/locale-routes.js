...
const LOCALE_CONFIG = [
  {
    ...
    sections: {
      ...
      privacy: { href: '/privacy/', segment: 'privacy/' },
      terms: { href: '/terms/', segment: 'terms/' },
    },
    content: {
  ...
  },
  {
    ...
    sections: {
      ...
      privacy: { href: '/pt/privacidade/', segment: 'privacidade/' },
      terms: { href: '/pt/termos/', segment: 'termos/' },
    },
    content: {
      'case-study': {
...
const SECTION_KEYS = new Set(['home', 'work', 'writing', 'about', 'privacy', 'terms']);
...
function getPermalinkPrefixes() {
  return new Map(
    getContentDirectories().map(({ locale, kind }) => [`${locale}:${kind}`, permalinkPrefix(locale, kind)])
  );
}
 ...
 */
function languageHref({ pages = [], translationKey, targetLocale, pageType, fallbackHref, writingIndexOther }) {
  if (translationKey) {
    const match = pages.find(
      (item) =>
        item.data.translationKey === translationKey && item.data.locale === targetLocale && item.data.draft !== true
    );
